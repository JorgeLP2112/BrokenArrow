...
const Home = () => {
    ...
    const posts = [{
        ...
        "images": [
            ...
            "https://www.google.com/url?sa=i&url=https%3A%2F%2Ftwitter.com%2FDatazoneC&psig=AOvVaw1ebSqijp9XjzOrKZr8SUcD&ust=1716163737455000&source=images&cd=vfe&opi=89978449&ved=0CBIQjRxqFwoTCIDh56G2mIYDFQAAAAAdAAAAABAJ"
        ]
    }]
                ...
                {posts.map((post, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-md mb-6 p-6">
                        ...
                        {post.images.map((image, index) => (
                            <img key={index} src={image} alt="" className="w-full h-64 object-cover mb-4 rounded" />
                        ))}
                        <button className="px-4 py-2 bg-blue-500 text-white rounded-md">Leer más</button>
                    </div>
                ))}
            </div>
        </div>
